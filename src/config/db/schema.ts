 ...
 * 存储 AI 生成的食谱和用户保存的食谱
 */
export const bbqRecipe = pgTable(
  'bbq_recipe',
  {
    id: text('id').primaryKey(),
    userId: text('user_id')
    ...
    title: text('title').notNull(),
    description: text('description'),
    prepTime: integer('prep_time'), // 准备时间（分钟）
    cookTime: integer('cook_time'), // 烹饪时间（分钟）
    difficulty: text('difficulty'), // easy, medium, hard
    servings: integer('servings').default(4),
    // 食材列表 JSON: [{name, amount, unit, price}]
    // amount 使用国际单位存储：重量用克(g)，容量用毫升(ml)
    ingredients: text('ingredients'),
    // 烹饪步骤 JSON: string[]
    instructions: text('instructions'),
    imageUrl: text('image_url'),
    imageQuery: text('image_query'), // 用于图片搜索的关键词
    language: text('language').default('en'), // 生成时的语言
  ...
  (table) => [
    // 查询用户的食谱列表
    index('idx_bbq_recipe_user_status').on(table.userId, table.status),
    // 按语言查询公开食谱
    index('idx_bbq_recipe_public_lang').on(table.isPublic, table.language),
    // 按创建时间排序
    index('idx_bbq_recipe_created_at').on(table.createdAt),
  ]
);
 ...
 * 用户收藏的食谱
 */
export const bbqFavorite = pgTable(
  'bbq_favorite',
  {
    id: text('id').primaryKey(),
    userId: text('user_id')
    ...
    recipeId: text('recipe_id')
      .notNull()
      .references(() => bbqRecipe.id, { onDelete: 'cascade' }),
    createdAt: timestamp('created_at').defaultNow().notNull(),
  },
  (table) => [
    // 查询用户的收藏列表
    index('idx_bbq_favorite_user').on(table.userId),
    // 检查是否已收藏（唯一约束替代）
    index('idx_bbq_favorite_user_recipe').on(table.userId, table.recipeId),
  ]
);
 ...
 * 记录用户查看过的食谱
 */
export const bbqRecipeHistory = pgTable(
  'bbq_recipe_history',
  {
    id: text('id').primaryKey(),
    userId: text('user_id')
  ...
  (table) => [
    // 查询用户的浏览历史（按时间倒序）
    index('idx_bbq_history_user_viewed').on(table.userId, table.viewedAt),
  ]
);
 ...
 * 用户的购物清单项目（持久化存储）
 */
export const bbqShoppingItem = pgTable(
  'bbq_shopping_item',
  {
    id: text('id').primaryKey(),
    userId: text('user_id')
      .notNull()
      .references(() => user.id, { onDelete: 'cascade' }),
    name: text('name').notNull(), // 食材名称
    amount: text('amount'), // 数量（字符串，如 "500"）
    unit: text('unit'), // 单位（g, ml, piece 等）
    price: integer('price'), // 价格（分为单位）
    recipeName: text('recipe_name'), // 来源食谱名称
    recipeId: text('recipe_id'), // 来源食谱 ID（可选）
    isBought: boolean('is_bought').default(false), // 是否已购买
    createdAt: timestamp('created_at').defaultNow().notNull(),
    updatedAt: timestamp('updated_at')
      .$onUpdate(() => new Date())
      .notNull(),
  ...
  (table) => [
    // 查询用户的购物清单
    index('idx_bbq_shopping_user').on(table.userId),
    // 查询未购买的项目
    index('idx_bbq_shopping_user_bought').on(table.userId, table.isBought),
  ]
);

/**
 * BBQ 菜单生成记录表
 * 记录 AI 菜单生成历史，用于统计和限额控制
 */
export const bbqMenuGeneration = pgTable(
  'bbq_menu_generation',
  {
    id: text('id').primaryKey(),
    userId: text('user_id')
    ...
    availableIngredients: text('available_ingredients'),
    language: text('language'),
    // 生成结果
    recipesCount: integer('recipes_count').default(0),
    // 消耗的 credits
    creditsUsed: integer('credits_used').default(1),
    creditId: text('credit_id'), // 关联的 credit 记录
  ...
  (table) => [
    // 查询用户的生成历史
    index('idx_bbq_generation_user').on(table.userId),
    // 按时间统计（用于限额检查）
    index('idx_bbq_generation_user_created').on(table.userId, table.createdAt),
  ]
);
